        ...
        if (stringLength==0) return 0; //When string length is 0
            ...
            if (checkAndReturnNegatives(numbers).size()>0)// When string has more than 1 negative numbers
                ...
                if (numbs[0].startsWith("//[")){
                    String escapedMetacharacter = escapeDanglingMetacharacter(numbs[0]);
                    int beginIndex = escapedMetacharacter.indexOf("[")+1;
                    int lastIndex = escapedMetacharacter.lastIndexOf("]");
                    String regex = escapedMetacharacter.substring(beginIndex, lastIndex);
